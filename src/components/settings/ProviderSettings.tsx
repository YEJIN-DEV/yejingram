        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between p-3 bg-[var(--color-bg-secondary)] rounded-lg border border-[var(--color-border)]">
                <div className="flex flex-col">
                    ...
                    <p className="text-xs text-[var(--color-text-tertiary)]">LLM이 응답 시간과 메시지를 직접 제어합니다. (권장)</p>
                    {provider === 'claude' && (
                        <p className="text-xs text-[var(--color-text-tertiary)] mt-1">주의: Claude의 경우 요청에 실패할 가능성이 있습니다.</p>
                    )}
                </div>
            ...
            {settings.useStructuredOutput && (
                <div className="flex items-center justify-between p-3 bg-[var(--color-bg-secondary)] rounded-lg border border-[var(--color-border)]">
                    <div className="flex flex-col">
                        ...
                        <p className="text-xs text-[var(--color-text-tertiary)]">대화 컨텍스트에 따라서 이미지 응답을 허용합니다.</p>
                    </div>
            ...
            {provider !== 'vertexai' && (
                <div>
                    <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><Key className="w-4 h-4 mr-2" />API 키</label>
                    <input
                        type="password"
                        value={config.apiKey}
            ...
            {provider === 'vertexai' && (
                <>
                    <div>
                        <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><Briefcase className="w-4 h-4 mr-2" />Project ID</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><Globe className="w-4 h-4 mr-2" />Location</label>
                        <input
                            type="text"
                    ...
                    <div>
                        <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><Key className="w-4 h-4 mr-2" />Access Token</label>
                        <input
                            type="password"
            ...
            {provider === 'customOpenAI' && (
                <div>
                    <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><Link className="w-4 h-4 mr-2" />Base URL</label>
                    <input
                        type="text"
            ...
            <div>
                <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><Cpu className="w-4 h-4 mr-2" />모델</label>
                    ...
                    <div className="grid grid-cols-1 gap-2 mb-3">
                        {models.map(model => (
                            <button
                                ...
                                className={`model-select-btn px-3 py-2 text-left text-sm rounded-lg transition-colors border ${config.model === model ? 'bg-blue-500 text-[var(--color-text-accent)] border-blue-500' : 'bg-[var(--color-bg-secondary)] text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-hover)] border-[var(--color-border)]'}`}>
                                {model}
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex gap-2">
                    <input
                        ...
                        className="flex-1 px-3 py-2 bg-[var(--color-bg-input-secondary)] text-[var(--color-text-primary)] rounded-lg border border-[var(--color-border)] focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 text-sm"
                    />
                    <button
                        type="button"
                        onClick={handleAddCustomModel}
                        className="px-4 py-2 bg-green-500 hover:bg-[var(--color-button-positive)] text-[var(--color-text-accent)] rounded-lg text-sm flex items-center gap-1">
                        ...
                        {config.customModels.map((model, index) => (
                            <div key={index} className="flex items-center gap-2">
                                <button
                                    type="button"
                                    onClick={() => handleModelSelect(model)}
                                    className={`model-select-btn flex-1 px-3 py-2 text-left text-sm rounded-lg transition-colors border ${config.model === model ? 'bg-blue-500 text-[var(--color-text-accent)] border-blue-500' : 'bg-[var(--color-bg-secondary)] text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-hover)] border-[var(--color-border)]'}`}>
                                    {model}
                                </button>
                                <button
                                    type="button"
            ...
            {settings.useImageResponse && (
                <>
                    <div>
                        <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><Key className="w-4 h-4 mr-2" />이미지 생성용 API 키</label>
                        <input
                            type="password"
                    ...
                    <div>
                        <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><Image className="w-4 h-4 mr-2" />이미지 생성 모델</label>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                {imageModels.map(model => (
                                    <button
                                        ...
                                        className={`model-select-btn px-3 py-2 text-left text-sm rounded-lg transition-colors border ${imageConfig.model === model ? 'bg-blue-500 text-[var(--color-text-accent)] border-blue-500' : 'bg-[var(--color-bg-secondary)] text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-hover)] border-[var(--color-border)]'}`}>
                                        {model}
                                    </button>
                                ))}
                            </div>
                    ...
                    </div>
                    {
                        imageProvider === 'gemini' && (
                            <div className="mb-3">
                                <p className="text-xs text-[var(--color-text-tertiary)]">참고: Gemini는 이미지 생성이 검열될 수 있습니다.</p>
                            </div>
                        )
                    }
                </>
            ...
            <div className="content-inner pt-4 space-y-4">
                <div>
                    <label className="flex items-center justify-between text-sm font-medium text-[var(--color-text-secondary)] mb-2">
                        ...
                        <span className="text-[var(--color-icon-accent)] font-semibold">{config.temperature || (provider !== 'claude' ? 1.25 : 1)}</span>
                    </label>
                    ...
                    <div className="flex justify-between text-xs text-[var(--color-text-tertiary)] mt-1"><span>{minTemp}</span><span>{maxTemp}</span></div>
                </div>
                <div>
                    <label className="flex items-center justify-between text-sm font-medium text-[var(--color-text-secondary)] mb-2">
                        <span className="flex items-center"><Percent className="w-4 h-4 mr-2" />Top-p</span>
                        <span className="text-[var(--color-icon-accent)] font-semibold">{config.topP || 1}</span>
                    </label>
                    ...
                    <div className="flex justify-between text-xs text-[var(--color-text-tertiary)] mt-1"><span>0</span><span>1</span></div>
                </div>
                {provider !== 'openai' && (
                    <div>
                        <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><Hash className="w-4 h-4 mr-2" />Top-k</label>
                        <input
                            type="number"
                ...
                {provider === 'claude' && (
                    <div>
                        <label className="flex items-center text-sm font-medium text-[var(--color-text-secondary)] mb-2"><ArrowUpToLine className="w-4 h-4 mr-2" />Max Tokens</label>
                        <input
                            type="number"
