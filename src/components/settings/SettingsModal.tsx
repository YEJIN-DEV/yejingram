...
function SettingsModal({ isOpen, onClose, openPromptModal }: SettingsModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md mx-4 flex flex-col shadow-2xl" style={{ maxHeight: '90vh' }}>
        ...
        <div className="p-6 space-y-2 overflow-y-auto">
          <details className="group border-b border-gray-200 pb-2">
            <summary className="flex items-center justify-between cursor-pointer list-none py-2">
              <span className="text-base font-medium text-gray-900">AI 설정</span>
          ...
          <details className="group border-b border-gray-200 pb-2">
            <summary className="flex items-center justify-between cursor-pointer list-none py-2">
              <span className="text-base font-medium text-gray-900">배율</span>
              <ChevronDown className="w-5 h-5 text-gray-400 transition-transform duration-300 group-open:rotate-180" />
            </summary>
            <div className="content-wrapper">
              <div className="content-inner pt-4 space-y-4">
          ...
          <details className="group border-b border-gray-200 pb-2">
            <summary className="flex items-center justify-between cursor-pointer list-none py-2">
              <span className="text-base font-medium text-gray-900">당신의 페르소나</span>
              <ChevronDown className="w-5 h-5 text-gray-400 transition-transform duration-300 group-open:rotate-180" />
            </summary>
            <div className="content-wrapper">
              <div className="content-inner pt-4 space-y-4">
                <div>
                  <label className="flex items-center text-sm font-medium text-gray-700 mb-2"><User className="w-4 h-4 mr-2" />당신을 어떻게 불러야 할까요?</label>
                  <input id="settings-user-name" type="text" placeholder="이름, 혹은 별명을 적어주세요" value={localSettings.userName} onChange={e => setLocalSettings(prev => ({ ...prev, userName: e.target.value }))} className="w-full px-4 py-3 bg-gray-50 text-gray-900 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all duration-200 text-sm" />
                </div>
                <div>
                  <label className="flex items-center text-sm font-medium text-gray-700 mb-2"><BrainCircuit className="w-4 h-4 mr-2" />당신은 어떤 사람인가요?</label>
                  <textarea id="settings-user-desc" placeholder="어떤 사람인지 알려주세요" value={localSettings.userDescription} onChange={e => setLocalSettings(prev => ({ ...prev, userDescription: e.target.value }))} className="w-full px-4 py-3 bg-gray-50 text-gray-900 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all duration-200 text-sm" rows={3}></textarea>
                </div>
              </div>
            </div>
          </details>
          <details className="group border-b border-gray-200 pb-2">
            <summary className="flex items-center justify-between cursor-pointer list-none py-2">
              <span className="text-base font-medium text-gray-900">선톡 설정</span>
              <ChevronDown className="w-5 h-5 text-gray-400 transition-transform duration-300 group-open:rotate-180" />
            </summary>
            <div className="content-wrapper">
              <div className="content-inner pt-4 space-y-4">
          ...
          <details className="group">
            <summary className="flex items-center justify-between cursor-pointer list-none py-2">
              <span className="text-base font-medium text-gray-900">데이터 관리</span>
              <ChevronDown className="w-5 h-5 text-gray-400 transition-transform duration-300 group-open:rotate-180" />
            </summary>
            <div className="content-wrapper">
        ...
        <div className="p-6 mt-auto border-t border-gray-200 shrink-0 flex justify-end space-x-3">
          <button onClick={onClose} className="flex-1 py-2.5 px-4 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition-colors">취소</button>
