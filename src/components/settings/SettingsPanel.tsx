...
function SettingsPanel({ openPromptModal, onClose }: SettingsPanelProps) {
    ...
    const [activeTab, setActiveTab] = useState<'ai' | 'scale' | 'persona' | 'proactive' | 'comfyui' | 'data'>('ai');
    ...
    return (
        <>
            <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} h-full flex flex-col border-r ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                <div className={`flex items-center justify-between p-6 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} shrink-0`}>
                    <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>설정</h3>
                    <button onClick={onClose} className={`p-1 ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'} rounded-full transition-colors`}><X className={`w-5 h-5 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`} /></button>
                </div>
                <div className="p-6 space-y-4 overflow-y-auto flex-1">
                    {/* Tab Navigation */}
                    <div className={`flex border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} -mx-6 px-6 whitespace-nowrap`}>
                        <button
                        ...
                        <button
                            ...
                            className={`py-3 px-4 text-sm font-medium border-b-2 transition-colors ${activeTab === 'comfyui'
                                ? (isDarkMode ? 'border-blue-400 text-blue-400' : 'border-blue-500 text-blue-600')
                                : (isDarkMode ? 'border-transparent text-gray-400 hover:text-gray-200' : 'border-transparent text-gray-500 hover:text-gray-700')
                                }`}
                        >
                    ...
                    <div className="mt-6">
                        {activeTab === 'ai' && (
                            <div className="space-y-4">
                                <div>
                                    <label className={`flex items-center text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}><Globe className="w-4 h-4 mr-2" />AI 제공업체</label>
                                    <select id="settings-api-provider" value={localSettings.apiProvider} onChange={handleProviderChange} className={`w-full px-4 py-3 ${isDarkMode ? 'bg-gray-700 text-gray-100 border-gray-600' : 'bg-gray-50 text-gray-900 border-gray-200'} rounded-xl border focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all duration-200 text-sm`}>
                                ...
                                <div>
                                    <label className={`flex items-center text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}><Image className="w-4 h-4 mr-2" />이미지 생성 제공업체</label>
                                    <select id="settings-image-api-provider" value={localSettings.imageApiProvider} onChange={handleImageProviderChange} className={`w-full px-4 py-3 ${isDarkMode ? 'bg-gray-700 text-gray-100 border-gray-600' : 'bg-gray-50 text-gray-900 border-gray-200'} rounded-xl border focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all duration-200 text-sm`}>
                                        <option value="gemini">Google Gemini</option>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className={`flex items-center justify-between text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>
                                        ...
                                        <span className={`${isDarkMode ? 'text-blue-400' : 'text-blue-500'} font-semibold`}>{localSettings.speedup}X</span>
                                    </label>
                                    <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} mt-2`}>* 구조화된 출력 사용중에는 적용되지 않습니다.</p>
                                    ...
                                    <div className={`flex justify-between text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} mt-1`}><span>느리게</span><span>빠르게</span></div>
                                </div>
                            </div>
                        )}
                        ...
                        {activeTab === 'proactive' && (
                            <div className="space-y-4">
                                <div className="py-2">
                                    <label className={`flex items-center justify-between text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} cursor-pointer`}>
                                        ...
                                        <div className="relative inline-block w-10 align-middle select-none">
                                            ...
                                            <label htmlFor="settings-proactive-toggle" className={`block overflow-hidden h-6 rounded-full ${isDarkMode ? 'bg-gray-600' : 'bg-gray-300'} cursor-pointer peer-checked:bg-blue-500`}></label>
                                ...
                                <div className={`py-2 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} mt-2 pt-2`}>
                                    <label className={`flex items-center justify-between text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} cursor-pointer`}>
                                        ...
                                        <div className="relative inline-block w-10 align-middle select-none">
                                            ...
                                            <label htmlFor="settings-random-first-message-toggle" className={`block overflow-hidden h-6 rounded-full ${isDarkMode ? 'bg-gray-600' : 'bg-gray-300'} cursor-pointer peer-checked:bg-blue-500`}></label>
                                            <span className="absolute left-0.5 top-0.5 block w-5 h-5 rounded-full bg-white transition-transform duration-200 ease-in-out peer-checked:translate-x-4"></span>
                                        </div>
                                    </label>
                                    {localSettings.randomFirstMessageEnabled && (
                                        <div id="random-chat-options" className="mt-4 space-y-4">
                                            <div>
                                                <label className={`flex items-center justify-between text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>
                                                    <span>생성할 인원 수</span>
                                                    <span id="random-character-count-label" className={`${isDarkMode ? 'text-blue-400' : 'text-blue-500'} font-semibold`}>{localSettings.randomCharacterCount}명</span>
                                                </label>
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <input id="settings-random-frequency-min" type="number" min="1" value={localSettings.randomMessageFrequencyMin} onChange={e => setLocalSettings(prev => ({ ...prev, randomMessageFrequencyMin: +e.target.value }))} className={`w-full px-3 py-2 ${isDarkMode ? 'bg-gray-700 text-gray-100 border-gray-600' : 'bg-gray-50 text-gray-900 border-gray-200'} rounded-lg border focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 text-sm`} placeholder="최소" />
                                                    <span className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>-</span>
                                                    <input id="settings-random-frequency-max" type="number" min="1" value={localSettings.randomMessageFrequencyMax} onChange={e => setLocalSettings(prev => ({ ...prev, randomMessageFrequencyMax: +e.target.value }))} className={`w-full px-3 py-2 ${isDarkMode ? 'bg-gray-700 text-gray-100 border-gray-600' : 'bg-gray-50 text-gray-900 border-gray-200'} rounded-lg border focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 text-sm`} placeholder="최대" />
                                                </div>
                                            </div>
                                        </div>
                                    )}
                        ...
                        {activeTab === 'data' && (
                            <div className="space-y-3">
                                ...
                                <button onClick={importBackup} id="restore-data-btn" className={`w-full py-2 px-4 ${isDarkMode ? 'bg-gray-700 hover:bg-gray-600 text-gray-300 border-gray-600' : 'bg-gray-100 hover:bg-gray-200 text-gray-700 border-gray-200'} rounded-lg transition-colors text-sm flex items-center justify-center gap-2 border`}>
                ...
                <div className={`p-6 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-200'} shrink-0`}>
