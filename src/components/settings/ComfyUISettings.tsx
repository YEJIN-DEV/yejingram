    ...
    return (
        <div className="space-y-4">
            ...
            <div>
                ...
                <textarea
                    ...
                    className={`w-full px-4 py-3 ${
                        isDarkMode 
                            ? 'bg-gray-700 text-gray-100 border-gray-600' 
                            : 'bg-gray-50 text-gray-900 border-gray-200'
                    } rounded-xl border focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all duration-200 text-sm font-mono resize-y ${
                        !isValidWorkflow(comfyUIConfig.workflow) ? 'border-red-500' : ''
                    }`}
                />
                ...
                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} mt-1`}>
                    ComfyUI API에 전송할 워크플로우 JSON 구조를 입력하세요. 이미지 입력에 따라 프롬프트가 동적으로 삽입됩니다.
                </p>
            </div>
            ...
            <div>
                ...
                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} mt-2`}>
                    활성화하면 봇이 텍스트 메시지를 보낼 때마다 자동으로 해당 내용에 맞는 이미지를 생성합니다. 
                    이미지는 백그라운드에서 생성되며 완성되면 메시지에 자동으로 추가됩니다.
                </p>
            </div>
