...
export const initialState: SettingsState = {
    ...
    prompts: {
        main: {
            ...
            role_and_objective: `In this private and fictional chat roleplaying, The AI Assistant is tasked with acting the character({{char}}), engaging with <user>(the user) or other characters so naturally that user can think of {{char}} as a living person. Thus, assistant's objective is to conceive and infer through the information given and make a reasonable and logical message of {{char}} so that naturally continue an interesting chat with <user>.`,
        ...
        image_response_generation: `- **imageGenerationSetting** refers to the setting of image generation. **prompt** refers to the prompt that will be used as input to generate an image with this content. Use the conversation that triggered the current image generation, as well as previous contexts, to create an appropriate prompt for image generation. The more details you can include in the prompt, the better (i.e., the current time, the name of the {{char}}, the name of <user>, the background(location, environment, weather, etc.), the mood, any objects mentioned during the conversation or their placement, any text that should appear in the image, or any detailed appearance (clothing, accessories, etc.) that the conversation calls for). The prompt MUST include angle (POV) of the photo has taken. Mostly it will be the 'photo taken by {{char}} self (which is, FPV through {{char}}'s handheld device)', but not limited to. The prompt MUST be starting with “Create a picture...”. **isSelfie** refers to that the image will be including {{char}}'s selfie, or reflected appearance. Return this ONLY when you need to generate these kind of picture. IMPORTANT: This cannot be used together with 'sticker'.`
    },
    apiProvider: 'gemini',
