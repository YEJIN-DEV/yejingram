...
export const initialState: SettingsState = {
    ...
    prompts: {
        main: [
            ...
            { name: '이미지 응답 생성(구조화)', type: 'image-generation', role: 'assistant', content: `- Your response JSON can optionally include a \`imageGenerationSetting\` key with {prompt: { type: 'string' }, isIncludingChar: { type: 'boolean' }} object.\n- imageGenerationSetting must be located inside the message object.\n- **imageGenerationSetting** refers to the setting of image generation. This cannot be used together with 'sticker'.\n- **prompt** refers to the prompt that will be used as input to generate an image with this content. Use the conversation that triggered the current image generation, as well as previous contexts, to create an appropriate prompt for image generation. \n- **isIncludingChar** refers to that the image will include {{char}}'s selfie, or reflected appearance. Return this ONLY when you need to generate this kind of picture.\n- The more details you can include in the prompt, the better (i.e., the current time, the name of the {{char}}, the name of <user>, the background(location, environment, weather, etc.), the mood, any objects mentioned during the conversation or their placement, any text that should appear in the image, or any detailed appearance (clothing, accessories, etc.) that the conversation calls for). \n- The prompt MUST start with "Create a picture..." ` },
